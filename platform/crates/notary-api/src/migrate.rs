...
pub async fn run_migrations() -> Result<()> {
    dotenvy::dotenv().ok();
    let database_url = env::var("NOTARY_API_MIGRATION_DATABASE_URL").context(
        "NOTARY_API_MIGRATION_DATABASE_URL must be set to a direct PostgreSQL connection URL",
    )?;
    let options = database_url
        .parse::<PgConnectOptions>()
        .context("NOTARY_API_MIGRATION_DATABASE_URL must be a PostgreSQL connection URL")?;
