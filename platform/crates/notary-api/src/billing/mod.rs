...
use super::{
    ApiError, ApiResult, BillingConfig, ErrorResponse, NotaryApiState, StripeConfig,
    admissions::{
        PurchaseAdjustmentSpec, apply_purchase_adjustment, grant_external_purchase,
        set_account_billing_state,
    },
    auth::authenticated_web_user,
    credits::{BillingStatus, Plan},
...
impl StripeClient {
    ...
    fn subscription_price_id(&self, plan: Plan) -> Result<&str> {
        match plan {
            Plan::OneGb => self
                .one_gb_price_id
                .as_deref()
    ...
    async fn retrieve_subscription_price(&self, plan: Plan) -> Result<StripeConfiguredPrice> {
...
async fn create_subscription_checkout_session(
    ...
    if super::admissions::account_billing_state(&state.database, &user.0)
        .await?
        .plan
        != Plan::Free
    {
...
fn checkout_return_urls(public_origin: &Url, purchase_id: &str) -> ApiResult<(Url, Url)> {
...
fn subscription_return_urls(public_origin: &Url) -> ApiResult<(Url, Url)> {
    let build = |outcome: &str| -> ApiResult<Url> {
        let mut url = public_origin
            .join("/")
            .map_err(|error| ApiError::internal(error.into()))?;
...
fn plan_for_price(stripe: &StripeClient, price_id: &str) -> ApiResult<Plan> {
...
fn parse_subscription_plan(value: &str) -> ApiResult<Plan> {
    match value {
...
mod tests {
    ...
    async fn subscription_status_and_price_drive_the_account_plan() {
        ...
        store_subscription(&state, "subscriber", Plan::OneGb, &subscription, now)
            .await
            .unwrap();
        let active = crate::admissions::account_billing_state(&state.database, "subscriber")
            .await
            .unwrap();
        ...
        store_subscription(&state, "subscriber", Plan::TenGb, &subscription, now + 2)
            .await
            .unwrap();
        let review = crate::admissions::account_billing_state(&state.database, "subscriber")
            .await
            .unwrap();
        ...
        store_subscription(&state, "subscriber", Plan::TenGb, &subscription, now + 3)
            .await
            .unwrap();
        let canceled = crate::admissions::account_billing_state(&state.database, "subscriber")
            .await
            .unwrap();
        ...
        store_subscription(&state, "subscriber", Plan::TenGb, &subscription, now + 5)
            .await
            .unwrap();
        let replaced = crate::admissions::account_billing_state(&state.database, "subscriber")
            .await
            .unwrap();
        ...
        let disputed = crate::admissions::account_billing_state(&state.database, "subscriber")
            .await
            .unwrap();
