...
fn describe_platform_metrics() {
    metrics::describe_counter!(
        "llm_notary_admission_jobs_total",
        "Publication admission attempts by outcome"
    );
    metrics::describe_histogram!(
        "llm_notary_admission_duration_seconds",
        "End-to-end publication admission duration"
    );
    metrics::describe_histogram!(
        "llm_notary_admission_archive_bytes",
        "Publication archive sizes admitted for verification"
    );
    metrics::describe_gauge!(
        "llm_notary_admission_queue_depth",
        "Publication jobs waiting for admission"
    );
    metrics::describe_gauge!(
        "llm_notary_admission_oldest_queued_seconds",
        "Age of the oldest queued publication job"
    );
}
...
mod tests {
    ...
    async fn admission_contract_migration_removes_only_legacy_surface() {
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0029_contract_legacy_admission.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
    ...
    async fn forward_migrations_remove_package_less_shares_after_preserving_compatibility() {
        ...
        for migration in [
            ...
            include_str!("../../../migrations/0002_library_card_facts.sql"),
            include_str!("../../../migrations/0003_admitted_library_card_facts.sql"),
            include_str!("../../../migrations/0004_service_admission.sql"),
        ] {
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0005_prepare_stamp_removal.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0006_remove_platform_stamp_contract.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0008_share_visibility_and_packages.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0009_credit_grants_and_ip_subjects.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        sqlx::raw_sql(include_str!(
            "../../../migrations/0010_restore_service_plan_rollback_compat.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        sqlx::raw_sql(include_str!(
            "../../../migrations/0011_share_library_previews.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0012_remove_package_less_shares.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0025_require_retained_share_packages.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0013_account_deletion_cascades.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
    ...
    async fn provider_neutral_identity_migration_backfills_and_dual_writes() {
        ...
        sqlx::raw_sql(include_str!(
            "../../../migrations/0016_provider_neutral_identities.sql"
        ))
        .execute(&database.pool)
        .await
        .unwrap();
    ...
    async fn provider_identity_cleanup_reconciles_and_preserves_account_ownership() {
        ...
        let error = sqlx::raw_sql(include_str!(
            "../../../migrations/0018_remove_legacy_github_identity.sql"
        ))
        .execute(&database.pool)
        .await
    ...
    async fn deleting_an_account_cascades_database_rows_and_queues_every_artifact() {
        ...
        sqlx::query(
            "INSERT INTO notary_credit_debits
             ...
             VALUES ('delete-debit', 'user:delete-user', 'delete-user', 'notarization', 40, 1)",
        )
        .execute(&pool)
        .await
        .unwrap();
