...
use super::{
    AppState, DatabasePool, ErrorResponse,
    hosted_verifier::{
...
impl VerificationPermits {
    async fn acquire(database: &DatabasePool, client: String) -> Result<Self, PermitError> {
        ...
        tokio::time::timeout(
            LIMITER_TIMEOUT,
            sqlx::query(
                "DELETE FROM anonymous_verification_leases
                 WHERE expires_at <= $1
                   AND client_key_sha256 IN (
                     SELECT client_key_sha256 FROM anonymous_verification_leases
                     WHERE expires_at <= $1
                     ORDER BY expires_at
                     LIMIT 64
                 )",
        ...
        let lease = tokio::time::timeout(
            ...
            sqlx::query_scalar::<_, String>(
                "INSERT INTO anonymous_verification_leases
                 ...
                 WHERE anonymous_verification_leases.expires_at <= $4
                 RETURNING lease_id",
            )
...
async fn verify_request(state: AppState, request: Request) -> Response {
    verify_request_with(state, request, run_worker_process).await
}

async fn verify_request_with<F, Fut>(state: AppState, request: Request, run_worker: F) -> Response
where
    F: FnOnce(Vec<u8>, NotaryDirectory) -> Fut,
    Fut: Future<Output = Result<Vec<u8>, WorkerError>>,
{
    ...
    let max_bytes = match usize::try_from(state.publish.max_archive_bytes) {
    ...
    let result = run_worker(archive, state.notary_directory.clone()).await;
    permits.release().await;
...
mod tests {
    ...
    async fn distributed_limiter_does_not_hold_a_pool_connection() {
        ...
        for _ in 0..super::super::config::DEFAULT_DATABASE_MAX_CONNECTIONS {
    ...
    async fn admission_and_anonymous_verification_share_capacity() {
        let _serial = TEST_SERIAL.lock().await;
        let state = test_state().await;
        let admission = super::super::hosted_verifier::acquire_verification_capacity().await;
        assert!(matches!(
        ...
        let anonymous = VerificationPermits::acquire(&state.database, "198.51.100.48".to_owned())
            ...
            .unwrap();
        assert!(
            super::super::hosted_verifier::try_acquire_verification_capacity().is_none(),
    ...
    async fn permit_acquisition_removes_expired_client_hashes() {
        ...
        let expired: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM anonymous_verification_leases
