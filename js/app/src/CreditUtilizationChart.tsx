...
import type { getCurrentUser } from './platform-api/client';
...
interface CreditUtilizationChartProps {
  ...
  historyDebits?: CreditHistoryEntry[] | null;
  historyError?: boolean;
}

function axisDayLabel(timestamp: number): string {
...
function fullDayLabel(timestamp: number): string {
  return new Intl.DateTimeFormat(undefined, {
    weekday: 'short',
    month: 'short',
    day: 'numeric',
...
export default function CreditUtilizationChart({
  credits,
  formatBytes,
  historyDebits = null,
  historyError = false,
  ...
  return (
    <section className="dashboard-utilization" aria-labelledby="dashboard-utilization-title">
      <header>
        <div>
          <span className="eyebrow">Notarization usage</span>
          <h2 id="dashboard-utilization-title">Last 30 days</h2>
        </div>
        <span>MB · UTC</span>
      </header>
      {historyError ? (
        <div
          className="dashboard-utilization-plot dashboard-utilization-plot--message"
          role="alert"
        >
          <strong>Daily utilization unavailable</strong>
          <span>Refresh the page to try again.</span>
        </div>
      ) : loading ? (
        <div
          className="dashboard-utilization-plot dashboard-utilization-plot--loading"
          role="status"
          ...
          <ResponsiveContainer width="100%" height={190}>
            ...
            >
              <XAxis
                ...
                dataKey="key"
                ticks={axisTicks}
                tickFormatter={(key) => axisLabels.get(key) || key}
                axisLine={false}
                tickLine={false}
                interval={0}
                fontSize={10}
              ...
              <YAxis
                type="number"
                domain={[0, maxMb > 0 ? 'auto' : 1]}
                axisLine={false}
                tickLine={false}
                width={64}
                tickFormatter={(value) => formatMb(Number(value))}
                fontSize={9}
                fill="var(--muted)"
                fontFamily="'IBM Plex Mono', monospace"
              />
              <Tooltip
                ...
                formatter={(value) => [formatMb(Number(value)), 'Utilization']}
                contentStyle={{
      ...
      <dl>
        <div>
          ...
          <dd>{loading || historyError ? '—' : formatMb(totalMb)}</dd>
        </div>
        <div>
          <dt>Available</dt>
          <dd>{formatBytes(notarization.total_remaining_bytes)}</dd>
        </div>
        <div>
          <dt>Overall budget</dt>
          <dd>{formatBytes(notarization.total_granted_bytes)}</dd>
        </div>
      </dl>
    </section>
  );
