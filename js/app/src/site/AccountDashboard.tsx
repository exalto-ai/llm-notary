...
type CliSessionPage = Awaited<ReturnType<typeof getCliSessions>>;
type CliSession = CliSessionPage['items'][number];
type SharePage = Awaited<ReturnType<typeof getMyShares>>;
type AccountShare = SharePage['items'][number];
type ShareSettings = Parameters<typeof updateMyShare>[1];
...
function shareStateLabel(state: AccountShare['state']): { label: string; tone: string } {
...
function shareAccessLabel(share: AccountShare): string {
...
interface ShareSettingsDialogProps {
  ...
  onSave: (share: AccountShare, settings: ShareSettings, closeSettings?: boolean) => Promise<void>;
  saving: boolean;
  error: string | null;
}
...
function ShareSettingsDialog({
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const settings: ShareSettings = { visibility };
  ...
  return (
    ...
    >
      <DialogContent className="axis-dialog share-settings-dialog">
        <DialogHeader>
          <DialogTitle>Manage trace {share.id.slice(0, 8)}</DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={submit}>
          ...
          {protectedAccess && (
            <label htmlFor="share-settings-password">
              <span>{share.password_protected ? 'New password (optional)' : 'Password'}</span>
              <Input
                ...
                placeholder={
                  share.password_protected ? 'Keep current password' : 'At least 8 characters'
                }
              />
            </label>
          )}
          ...
          {share.expires_at && expiryMode === 'keep' && (
            <p className="share-settings-current">
              Currently expires {sessionDate(share.expires_at)}.
            </p>
          )}
          {error && (
...
interface DashboardProps {
  ...
  loadCliSessions?: typeof getCliSessions;
  loadMyShares?: typeof getMyShares;
  ...
  updateShareRequest?: typeof updateMyShare;
  route?: string;
...
export function Dashboard({
  ...
  const [revokeTarget, setRevokeTarget] = useState<CliSession | null>(null);
  ...
  const [shareSettingsTarget, setShareSettingsTarget] = useState<AccountShare | null>(null);
  const [unpublishTarget, setUnpublishTarget] = useState<AccountShare | null>(null);
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && revoking !== revokeTarget.id) setRevokeTarget(null);
    };
  ...
  const revoke = async (session: CliSession) => {
  ...
  return (
    <main className="dashboard-shell dashboard-shell--account">
              ...
              <section className="dashboard-traces" aria-label="Your traces">
                ...
                {shares === null && !shareError ? (
                  ...
                  <div className="dashboard-trace-list">
                    {shares.map((share) => {
                      const status =
                        share.state === 'admitted' && share.published === false
                          ? { label: 'Unpublished', tone: 'neutral' }
                          : shareStateLabel(share.state);
                      return (
                        <article key={share.id}>
                          <div className="dashboard-trace-copy">
                            ...
                            <p>
                              ...
                              {share.expires_at
                                ? ` · Expires ${sessionDate(share.expires_at)}`
                                : ''}{' '}
                              · <code>{share.id}</code>
                            </p>
                            {share.failure_code && (
                              <p className="dashboard-trace-failure">
                                Reason: {share.failure_code.replaceAll('_', ' ')}
                              </p>
                            )}
                          </div>
                          <div className="dashboard-trace-actions">
                            ...
                            {share.package_url && !share.password_protected && (
                              <a href={share.package_url}>Package</a>
                            )}
                            {share.state === 'admitted' && (
                              <button
                                type="button"
                                onClick={() => {
                                  setShareError(null);
                                  setShareSettingsTarget(share);
                                }}
                              >
                                Manage
                              </button>
                            ...
                            {share.state === 'admitted' && share.published === false && (
                              <button
                                type="button"
                                onClick={() => saveShareSettings(share, { published: true }, false)}
                                disabled={savingShare === share.id}
                              >
                                {savingShare === share.id ? 'Publishing…' : 'Publish again'}
                              </button>
                            )}
                          </div>
                        </article>
      ...
      <ShareSettingsDialog
        ...
        onOpenChange={(nextOpen) => {
          if (!nextOpen) setShareSettingsTarget(null);
        }}
        onSave={saveShareSettings}
        saving={savingShare === shareSettingsTarget?.id}
        error={shareError}
      />
      <AlertDialog
      ...
      >
        <AlertDialogContent className="axis-alert-dialog">
          <AlertDialogHeader>
            <AlertDialogTitle>Unpublish trace {unpublishTarget?.id.slice(0, 8)}?</AlertDialogTitle>
            <AlertDialogDescription>
              The stable link, trace JSON, and package will stop working immediately. You can
              publish this trace again later.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={Boolean(savingShare)}>Keep published</AlertDialogCancel>
            <AlertDialogAction
              disabled={Boolean(savingShare)}
              onClick={() =>
                unpublishTarget && saveShareSettings(unpublishTarget, { published: false }, false)
              }
            >
              {savingShare ? 'Unpublishing…' : 'Unpublish trace'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
