...
const docPages = {
  ...
  'getting-started': {
    ...
    blocks: [
      ...
      { heading: 'Open the local dashboard', body: 'Visit http://127.0.0.1:8788, sign in with the private admin token created at admin.token_path, and use the sidebar for captures, finalizations, trace verification, publishing, activity, and settings. Appearance controls live on Settings; the dashboard does not repeat listener status in a separate header.' },
      ...
      { heading: 'Search local captures', body: 'Each completed provider interaction keeps its encrypted bundle and gains a row in the local SQLite catalog. Use the dashboard, or fetch the live OpenAPI document and query the authenticated admin API:', code: 'curl -H "Authorization: Bearer $LLM_NOTARY_ADMIN_TOKEN" \\\n  "http://127.0.0.1:8788/v1/captures?query=pricing&model=gpt-5"' },
  ...
  'trace-packages': {
    ...
    blocks: [
      { heading: 'Finalize one interaction', body: 'Select a pending capture identifier in the dashboard or admin API. The service writes to storage.finalized_dir/<capture-id> by default, records that package in the catalog, and retains the encrypted source bundle.', code: 'curl -X POST -H "Authorization: Bearer $LLM_NOTARY_ADMIN_TOKEN" \\\n  http://127.0.0.1:8788/v1/captures/cap-example/finalizations' },
