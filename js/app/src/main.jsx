    ...
    blocks: [
      ...
      { heading: 'What happens at end-of-stream', body: 'The proxy seals encrypted deferred state into one `.llmcapture`. It does not perform the expensive final proof before returning control to your workflow.' },
      {
        ...
        definitions: [
          ...
          { term: 'Codex with a ChatGPT plan', description: 'Add a Codex custom provider with base URL http://127.0.0.1:8787/codex and requires_openai_auth = true. Codex keeps and attaches its own ChatGPT login.' },
          { term: 'Anthropic / Claude Code', description: 'Set the base URL to http://127.0.0.1:8787/anthropic and continue sending Messages API requests. Claude Code can retain either its API key or its own saved claude.ai login.' },
      ...
      { heading: 'Codex + ChatGPT plan', body: 'First run codex login status and confirm that Codex says it is logged in using ChatGPT. Do not set env_key in this provider. Codex owns the login and refreshes it; LLM Notary only forwards the request headers and hides every header value from the finalized package. The proof authenticates chatgpt.com and the disclosed bodies, not the account owner, plan, or billing.', code: 'Add this to ~/.codex/config.toml:\n\nmodel_provider = "llm-notary-chatgpt"\n\n[model_providers.llm-notary-chatgpt]\nname = "LLM Notary — ChatGPT plan"\nbase_url = "http://127.0.0.1:8787/codex"\nrequires_openai_auth = true\nwire_api = "responses"\nsupports_websockets = false' },
