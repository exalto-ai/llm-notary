...
function Docs({ pageKey }) {
  ...
  const isCommand = (text) => text.startsWith('curl ') || text.startsWith('llm-notary ');
  ...
  return <main className="docs-shell"><aside className="docs-sidebar"><span className="eyebrow">Documentation</span><a className={!pageKey || pageKey === 'overview' ? 'active' : ''} href="#/docs">Overview</a><a className={pageKey === 'how-it-works' ? 'active' : ''} href="#/docs/how-it-works">Trust model</a><span className="docs-group">CLI</span><a className={pageKey === 'install' ? 'active' : ''} href="#/docs/install">Install</a><a className={pageKey === 'proxy' ? 'active' : ''} href="#/docs/proxy">Proxy</a><a className={pageKey === 'publish' ? 'active' : ''} href="#/docs/publish">Publish</a><a className={pageKey === 'verify' ? 'active' : ''} href="#/docs/verify">Verify</a><a href="#/collections">Collections</a></aside><article className="docs-content"><span className="eyebrow">LLM Notary / Docs</span><h1>{page.title}</h1><p className="docs-lead">{page.lead}</p>{page.blocks.map(([heading, body]) => <section key={heading}><h2>{heading}</h2>{isCommand(body) ? <pre><code>{body}</code></pre> : <p>{body}</p>}</section>)}<div className="docs-next"><span>Next</span><a href={next.href}>{next.label}</a></div></article></main>;
}
