    ...
    blocks: [
      ...
      { heading: 'What happens at end-of-stream', body: 'The proxy seals encrypted deferred state into one `.llmcapture`. It does not perform the expensive final proof before returning control to your workflow.' },
      {
        ...
        definitions: [
          ...
          { term: 'Codex with a ChatGPT plan', description: 'Live-tested with Codex CLI. Add a custom provider with base URL http://127.0.0.1:8787/codex and requires_openai_auth = true. Codex keeps and attaches its own ChatGPT login.' },
          { term: 'Anthropic / Claude Code', description: 'Live-tested with Claude Code. Set the base URL to http://127.0.0.1:8787/anthropic and continue sending Messages API requests with either its API key or its own saved claude.ai login.' },
      ...
      { heading: 'Provider boundary', body: 'The first local path segment selects a fixed adapter: /openai, /codex, /anthropic, /deepseek, or /openrouter. Each adapter fixes the upstream hostname and path to an explicit allowlist. The notary—not a caller-supplied URL—resolves and opens the provider connection.' },
      { heading: 'Codex + OpenAI API key', body: 'Replace YOUR_RESPONSES_MODEL with a model available to the OpenAI API key. The explicit no-WebSocket capability keeps Codex on this prototype\'s supported HTTP transport.', code: 'Add this to ~/.codex/config.toml:\n\nmodel_provider = "llm-notary"\nmodel = "YOUR_RESPONSES_MODEL"\n\n[model_providers.llm-notary]\nname = "LLM Notary local proxy"\nbase_url = "http://127.0.0.1:8787/openai/v1"\nenv_key = "OPENAI_API_KEY"\nwire_api = "responses"\nsupports_websockets = false' },
      { heading: 'Codex + ChatGPT plan', body: 'This flow is live-tested with Codex CLI. First run codex login status and confirm that Codex says it is logged in using ChatGPT. Do not set env_key in this provider. Codex owns the login and refreshes it; LLM Notary only forwards the request headers and hides every header value from the finalized package. The proof authenticates chatgpt.com and the disclosed bodies, not the account owner, plan, or billing. Codex desktop is not yet end-to-end tested or supported, and cloud work cannot reach the loopback route.', code: 'Add this to ~/.codex/config.toml:\n\nmodel_provider = "llm-notary-chatgpt"\n\n[model_providers.llm-notary-chatgpt]\nname = "LLM Notary — ChatGPT plan"\nbase_url = "http://127.0.0.1:8787/codex"\nrequires_openai_auth = true\nwire_api = "responses"\nsupports_websockets = false' },
      { heading: 'Run Codex', code: 'codex exec --ephemeral --skip-git-repo-check \\\n  \'Reply with exactly: hello\'' },
      { heading: 'Claude Code + Anthropic API key', body: 'Set Claude Code\'s Anthropic base URL to the local route, keep the API key in its normal environment, and choose a model available to that account.', code: 'ANTHROPIC_BASE_URL=http://127.0.0.1:8787/anthropic \\\nclaude --bare --no-session-persistence \\\n  -p --model YOUR_MODEL \\\n  \'Reply with exactly: hello\'' },
      { heading: 'Claude Code + claude.ai plan', body: 'Run claude auth status first; Claude Code itself must report a saved first-party login. A login in the native Claude desktop app is separate. Remove apiKeyHelper and do not set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN. Claude Code owns login and refresh; LLM Notary forwards its authorization and Messages protocol unchanged and hides every header value from the finalized package. The proof authenticates api.anthropic.com and the disclosed bodies, not the account owner, subscription, or billing. Native Claude Desktop, web, Slack, remote, and cloud sessions do not run through this local route.', code: 'env -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN \\\n  ANTHROPIC_BASE_URL=http://127.0.0.1:8787/anthropic \\\n  claude -p \'Reply with exactly: llm-notary\'' },
      ...
      { heading: 'Search local captures', body: 'Each completed provider interaction keeps its vault-encrypted bundle in the selected private artifact store and gains a row in the selected metadata store. Use the dashboard, or fetch the live OpenAPI document and query the local admin API:', code: 'curl "http://127.0.0.1:8788/v1/captures?query=pricing&provider=openai"' },
  ...
  return <main className="share-page">
    ...
    <div className="share-page-layout">
      ...
      <aside className="share-evidence-rail"><span className="eyebrow">Verification</span><dl><div><dt>Provider</dt><dd><ProviderIdentity provider={share.provider} /></dd></div><div><dt>Host</dt><dd>{share.host}</dd></div><div><dt>Authenticated</dt><dd>{authenticated}</dd></div><div><dt>Visibility</dt><dd>{share.visibility}{share.password_protected ? ' · password protected' : ''}</dd></div>{share.expires_at && <div><dt>Expires</dt><dd>{sessionDate(share.expires_at)}</dd></div>}</dl>
        {share.password_protected ? <button className="share-package-download" type="button" onClick={saveProtectedPackage}><span>Package</span><b>Download .llmtrace</b><small>{binaryFileSize(share.package_size_bytes)} · SHA-256 included</small></button> : <a className="share-package-download" href={share.package_url}><span>Package</span><b>Download .llmtrace</b><small>{binaryFileSize(share.package_size_bytes)} · SHA-256 included</small></a>}
        {packageError && <p className="share-package-error" role="alert">{packageError}</p>}
        <details className="share-technical"><summary>Hashes and notary</summary><dl><div><dt>Trace SHA-256</dt><dd><code>{share.trace_sha256}</code></dd></div><div><dt>Package SHA-256</dt><dd><code>{share.package_sha256}</code></dd></div><div><dt>Notary key</dt><dd><code>{share.notary_key_id || 'Not recorded'}</code></dd></div><div><dt>Directory generation</dt><dd>{share.directory_generation ?? 'Not recorded'}</dd></div><div><dt>Safety contract</dt><dd><code>{share.public_package_safety_version}</code></dd></div></dl></details>
