...
export function ProviderIdentity({
  ...
  return <span className={`provider-identity ${className}`.trim()} data-provider-icon={key}>
    {iconUrl
      ? <span className="provider-identity__icon" style={iconStyle} aria-hidden="true" />
      : <svg className="provider-identity__icon provider-identity__icon--unknown" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path d="M4.5 7.5 12 3l7.5 4.5v9L12 21l-7.5-4.5v-9Z" />
        <circle cx="12" cy="12" r="2.5" />
      </svg>}
    <span className="provider-identity__name">{label}</span>
    {detail !== undefined && detail !== null && <span className="provider-identity__detail">{detail}</span>}
  </span>;
}
