...
pub async fn run(args: PublishArgs) -> Result<()> {
    ...
    let manifest = verify_trace_package(&args.package, &trusted_notary_key)
...
async fn submit_archive(
    ...
    if created.job.state == "uploading" {
        ...
        upload_archive(&client, upload, archive).await?;
        api_json::<PublishJob>(
            client
                .post(format!(
...
async fn upload_archive(
    ...
    let url = url::Url::parse(&instructions.url)
        .context("publication API returned an invalid upload URL")?;
