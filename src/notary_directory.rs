...
pub struct NotaryDirectoryRecord {
    ...
    /// Last wall-clock time at which this endpoint may finalize an existing
    /// deferred bundle. This is separate from the capture cutoff.
    pub finalize_until_unix_ms: Option<u64>,
}
...
fn validate_record(record: &NotaryDirectoryRecord) -> Result<()> {
    ...
    if record
        ...
        .is_some_and(|until| until < record.valid_from_unix_ms)
    {
        bail!("directory key finalization window is inverted");
    }
    if record.status == NotaryKeyStatus::Retiring {
        ...
        let finalize_until = record
            .finalize_until_unix_ms
            .ok_or_else(|| anyhow::anyhow!("a retiring key requires a finalization cutoff"))?;
        if finalize_until < capture_until {
            bail!("a retiring key cannot stop finalization before its capture cutoff");
        }
    }
...
mod tests {
    ...
    #[test]
    fn retiring_key_separates_capture_and_finalization_cutoffs() {
        let active = record(8, NotaryKeyStatus::Active);
        ...
        assert!(retiring.accepts_finalization_at(250));
        assert!(!retiring.accepts_finalization_at(300));
    }
