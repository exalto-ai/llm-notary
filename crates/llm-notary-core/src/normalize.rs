...
pub fn render_public_trace(inferences: &[VerifiedInference]) -> Result<Vec<u8>> {
    ...
    let conversation_id = format!("llmnotary:{trace_id}");
    ...
    let trace = json!({
        "resourceSpans": [{
            "resource": {"attributes": [
                string_attribute("llmnotary.format", PUBLIC_TRACE_FORMAT),
                string_attribute("llmnotary.normalizer.version", NORMALIZER_VERSION),
                string_attribute("otel.semconv.version", OTEL_SEMCONV_VERSION),
                string_attribute("service.name", "llm-notary")
            ]},
            "scopeSpans": [{
                "scope": {"name": "llm-notary.normalizer", "version": NORMALIZER_VERSION},
                "spans": spans
            }]
        }]
    });
...
mod tests {
    use super::*;
    use crate::{CaptureArtifacts, CaptureNotary, CaptureProvider};

    fn manifest(provider: &str) -> CaptureManifest {
        CaptureManifest {
            format: crate::CAPTURE_FORMAT.to_owned(),
            capture_id: format!("cap-{provider}"),
