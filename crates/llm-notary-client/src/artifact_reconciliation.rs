...
use crate::{
    ...
    metadata::{CaptureFilters, CapturePagePosition},
    ...
    loop {
        ...
        for capture in &captures {
            referenced.extend(
                persistence
                    .metadata
                    .artifacts(&capture.capture_id)
                    .await
                    .context("listing artifact metadata for reconciliation")?,
            );
        }
        if captures.len() < CAPTURE_PAGE_SIZE {
            break;
        }
        cursor = captures.last().map(CapturePagePosition::from);
    }
...
mod tests {
    ...
    async fn reports_verified_and_missing_files_without_mutating_metadata() {
        ...
        let capture_id = "cap-reconciliation";
        persistence
            .metadata
