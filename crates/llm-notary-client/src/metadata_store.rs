...
//!
//! The daemon, administration API, and finalization worker use this module
//! rather than calling SQLite directly. Blocking SQLite work is contained by
//! [`SqliteMetadataStore`]; later backends can implement the same contract
...
use crate::{
    ...
    catalog::{
        CaptureFilters as SqliteCaptureFilters, Catalog, EventFilters as SqliteEventFilters,
        OperationFilters as SqliteOperationFilters,
    },
    metadata::{
...
pub trait MetadataStore: Send + Sync {
    ...
    async fn capturing_ids(&self) -> MetadataResult<Vec<String>>;
    ...
    async fn operations_for_capture(&self, capture_id: &str) -> MetadataResult<Vec<Operation>>;
...
impl SqliteMetadataStore {
    pub async fn open(path: PathBuf, full_text_search: bool) -> MetadataResult<Self> {
        let catalog = tokio::task::spawn_blocking(move || Catalog::open(&path, full_text_search))
            .await
    ...
    #[cfg(test)]
    pub(crate) fn from_catalog(catalog: Catalog, full_text_search: bool) -> Self {
        Self {
            catalog: Arc::new(catalog),
            full_text_search,
        }
    ...
    where
        T: Send + 'static,
        F: FnOnce(&Catalog) -> anyhow::Result<T> + Send + 'static,
    {
...
impl MetadataStore for SqliteMetadataStore {
    ...
    async fn capturing_ids(&self) -> MetadataResult<Vec<String>> {
        self.blocking(Catalog::capturing_ids).await
    }
    ...
    async fn captures(&self, filters: CaptureFilters) -> MetadataResult<Vec<CaptureSummary>> {
        ...
        self.blocking(move |catalog| {
            catalog.filtered_captures(&SqliteCaptureFilters {
                ...
                created_after_unix_ms: filters.created_after_unix_ms,
                cursor: filters.cursor.as_ref(),
                limit: filters.limit,
            })
        })
        .await
    ...
    async fn counts(&self) -> MetadataResult<MetadataCounts> {
        self.blocking(Catalog::counts).await
    }
    ...
    async fn operations(&self, filters: OperationFilters) -> MetadataResult<Vec<Operation>> {
        ...
        self.blocking(move |catalog| {
            catalog.filtered_operations(&SqliteOperationFilters {
    ...
    async fn operations_for_capture(&self, capture_id: &str) -> MetadataResult<Vec<Operation>> {
        let capture_id = capture_id.to_owned();
        self.blocking(move |catalog| catalog.operations_for_capture(&capture_id))
            .await
    }
    ...
    async fn events_snapshot(&self, filters: EventFilters) -> MetadataResult<EventSnapshot> {
        ...
        self.blocking(move |catalog| {
            let (events, high_water) =
                catalog.filtered_events_with_high_water(&SqliteEventFilters {
...
mod tests {
    ...
    async fn sqlite_adapter_migrates_schema_versions_one_through_six() {
        ...
        for version in 1..=6 {
            ...
            crate::catalog::create_schema_fixture(&database, version).unwrap();
