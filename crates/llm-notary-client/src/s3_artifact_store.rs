...
impl S3ArtifactStore {
    ...
    fn object_key(&self, key: &ArtifactKey) -> String {
        let (kind, extension) = match key.kind() {
            ArtifactKind::DeferredBundle => ("deferred_bundle", "llmcapture"),
            ArtifactKind::FinalizedPackage => ("finalized_package", "llmtrace"),
        };
        let suffix = format!("daemon-private/{kind}/{}.{extension}", key.capture_id());
        if self.prefix.is_empty() {
            suffix
        } else {
    ...
    fn scoped_object_key(&self, key: &ArtifactKey, publication_id: &str) -> ArtifactResult<String> {
        let publication_id = uuid::Uuid::parse_str(publication_id)
            .map_err(|error| invalid("invalid_publication_id", anyhow!(error)))?;
        let (kind, extension) = match key.kind() {
            ArtifactKind::DeferredBundle => ("deferred_bundle", "llmcapture"),
            ArtifactKind::FinalizedPackage => ("finalized_package", "llmtrace"),
        };
...
mod tests {
    ...
    fn key_and_locator_are_safe_deterministic_and_backend_tagged() {
        ...
        let key = ArtifactKey::new("cap-safe-123", ArtifactKind::DeferredBundle).unwrap();
        let object_key = store.object_key(&key);
        assert_eq!(
            object_key,
            "tenant_1/private-artifacts/daemon-private/deferred_bundle/cap-safe-123.llmcapture"
        );
        ...
        let scoped = store
            .scoped_object_key(&record.key, publication_id)
            .unwrap();
        assert_eq!(
            scoped,
            "tenant_1/private-artifacts/daemon-private/deferred_bundle/cap-safe-123/123e4567-e89b-12d3-a456-426614174000.llmcapture"
        );
        ...
        assert_eq!(
            ...
            "daemon-private/deferred_bundle/cap-safe-123.llmcapture"
        );
        let finalized = ArtifactKey::new("cap-safe-123", ArtifactKind::FinalizedPackage).unwrap();
        assert_eq!(
    ...
    async fn ambiguous_put_waits_for_a_delayed_committed_object() {
        ...
        let key = ArtifactKey::new("cap-delayed-commit", ArtifactKind::DeferredBundle).unwrap();
    ...
    async fn minio_conforms_and_detects_missing_corrupt_and_oversized_objects() {
        ...
        .unwrap();
        let s3_key = ArtifactKey::new("cap-s3-switch", ArtifactKind::DeferredBundle).unwrap();
        ...
        let filesystem_key =
            ArtifactKey::new("cap-filesystem-switch", ArtifactKind::FinalizedPackage).unwrap();
        ...
        let missing_key = ArtifactKey::new("cap-s3-deleted", ArtifactKind::DeferredBundle).unwrap();
        ...
        let corrupt_key =
            ArtifactKey::new("cap-s3-corrupt", ArtifactKind::FinalizedPackage).unwrap();
        ...
        let oversized_key =
            ArtifactKey::new("cap-s3-oversized", ArtifactKind::DeferredBundle).unwrap();
        ...
        let referenced_record = inventory_store
            ...
            .unwrap();
        let orphan_key =
            ArtifactKey::new("cap-s3-unreferenced", ArtifactKind::DeferredBundle).unwrap();
        inventory_store
            .put(
                &orphan_key,
