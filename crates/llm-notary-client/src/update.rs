//! Authenticated release checks and rollback-safe command-line installation.
...
use crate::cli::{BUILD_ID, DEFAULT_PUBLIC_ORIGIN, UPDATES_ENABLED};
...
#[derive(Clone, Debug)]
pub(crate) struct BackgroundUpdateStatus {
    pub(crate) enabled: bool,
    pub(crate) current_build_id: String,
    pub(crate) latest_build_id: Option<String>,
    pub(crate) update_available: bool,
    pub(crate) last_checked_unix_ms: Option<u64>,
    pub(crate) error_code: Option<String>,
}

pub(crate) type SharedUpdateStatus = Arc<RwLock<BackgroundUpdateStatus>>;

pub(crate) fn background_status() -> SharedUpdateStatus {
...
fn accept_channel_revision(
    ...
    crate::cli::storage::write_private_file_atomically(path, &bytes)
...
fn write_journal(install: &InstallPaths, build_id: &str, phase: JournalPhase) -> Result<()> {
    ...
    crate::cli::storage::write_private_file_atomically(&install.journal, &bytes)?;
    sync_directory(&install.directory)
}
...
pub fn run_windows_apply_helper(
    ...
    if let Ok(bytes) = serde_json::to_vec(&report) {
        let _ = crate::cli::storage::write_private_file_atomically(&report_path, &bytes);
    }
    result
}
