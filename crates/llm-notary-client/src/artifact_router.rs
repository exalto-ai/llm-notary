...
mod tests {
    ...
    fn record(locator: &str, sha256: &str) -> ArtifactRecord {
        ArtifactRecord::new(
            ArtifactKey::new("cap-router", ArtifactKind::DeferredBundle).unwrap(),
    ...
    #[tokio::test]
    async fn legacy_filesystem_locators_remain_readable() {
        let (_directory, router) = filesystem_router();
        let key = ArtifactKey::new("cap-router-legacy", ArtifactKind::DeferredBundle).unwrap();
        let record = router
    ...
    fn recovery_rejects_cross_backend_collisions() {
        ...
        let filesystem = record("/managed/bundle.llmcapture", &digest);
