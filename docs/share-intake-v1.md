...
An owner can explicitly change discoverability without changing the stable
link:
...
`trace.otlp.json` and the exact admitted `.llmtrace` package use immutable,
content-addressed public keys. The database records each object key, byte size,
