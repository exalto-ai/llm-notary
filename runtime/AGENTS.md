...
Run `./tooling/check-boundary.sh`, `cargo fmt --check -p notary-core -p notaryd -p llm-notary-cli -p llm-notary-updater -p notary-server`, `cargo test --workspace --all-targets --all-features`, and `npm --prefix apps/local-dashboard run build` for changes that affect the corresponding code.
