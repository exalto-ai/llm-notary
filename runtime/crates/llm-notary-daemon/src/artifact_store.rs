...
pub enum ArtifactKind {
    /// A vault-encrypted deferred `.llmcapture` checkpoint.
    DeferredBundle,
...
impl ArtifactKind {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DeferredBundle => "deferred_bundle",
            Self::FinalizedPackage => "finalized_package",
        }
    }

    const fn extension(self) -> &'static str {
        match self {
            Self::DeferredBundle => "llmcapture",
            Self::FinalizedPackage => "llmtrace",
        }
    }
}
...
impl TryFrom<&str> for ArtifactKind {
    ...
    fn try_from(value: &str) -> Result<Self> {
        match value {
            "deferred_bundle" => Ok(Self::DeferredBundle),
            "finalized_package" => Ok(Self::FinalizedPackage),
...
impl ArtifactKey {
    ...
    pub fn new(capture_id: impl Into<String>, kind: ArtifactKind) -> Result<Self> {
        let capture_id = capture_id.into();
        validate_capture_id(&capture_id)?;
        Ok(Self { capture_id, kind })
    }
...
impl FileSystemArtifactStore {
    pub fn new(bundle_dir: impl Into<PathBuf>, finalized_dir: impl Into<PathBuf>) -> Self {
        Self {
            bundle_dir: bundle_dir.into(),
            finalized_dir: finalized_dir.into(),
        }
    }
    ...
    fn current_path(&self, key: &ArtifactKey) -> Result<PathBuf> {
        ...
        Ok(self
            .root(key.kind())
            .join(format!("{}.{}", key.capture_id(), key.kind().extension())))
    }

    fn legacy_path(&self, key: &ArtifactKey) -> Result<Option<PathBuf>> {
        if key.kind() != ArtifactKind::DeferredBundle {
            return Ok(None);
        }
        validate_root(&self.bundle_dir)?;
        Ok(Some(
            self.bundle_dir
                .join(format!("{}.llmbundle", key.capture_id())),
        ))
    }

    fn root(&self, kind: ArtifactKind) -> &Path {
        match kind {
            ArtifactKind::DeferredBundle => &self.bundle_dir,
            ArtifactKind::FinalizedPackage => &self.finalized_dir,
        }
    }
    ...
    fn checked_record_path(&self, record: &ArtifactRecord) -> ArtifactResult<PathBuf> {
        ...
        if self
            .legacy_path(&record.key)
            .map_err(|error| invalid("invalid_storage_root", error))?
            .as_deref()
            .is_some_and(|legacy| locator_path == legacy)
        {
            return Ok(locator_path.to_owned());
        }
        Err(invalid(
            "invalid_locator",
            anyhow!(
                "artifact locator does not belong to capture {:?} and kind {}",
                record.key.capture_id(),
    ...
    async fn find(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if let Some(legacy) = legacy {
                match inspect_artifact(&legacy, key, max_bytes) {
                    Ok(record) => return Ok(Some(record)),
                    Err(ArtifactStoreError::NotFound { .. }) => {}
                    Err(error) => return Err(error),
                }
...
mod tests {
    ...
    fn key(kind: ArtifactKind) -> ArtifactKey {
        ArtifactKey::new("cap-artifact-test", kind).unwrap()
    }
    ...
    #[tokio::test]
    async fn discovers_and_reads_legacy_llmbundle() {
        let (directory, store) = fixture();
        let bundle_dir = directory.path().join("bundles");
        fs::create_dir_all(&bundle_dir).unwrap();
        let legacy_path = bundle_dir.join("cap-artifact-test.llmbundle");
    ...
    #[tokio::test]
    async fn rejects_capture_ids_and_locators_that_could_escape_the_root() {
    ...
    async fn bounded_find_rejects_oversized_sparse_artifact_without_reading_it() {
        ...
        let bundle_dir = directory.path().join("bundles");
        fs::create_dir_all(&bundle_dir).unwrap();
        let path = bundle_dir.join("cap-artifact-test.llmcapture");
        File::create(&path)
            .unwrap()
        ...
        assert!(matches!(
            store
                .find(&key(ArtifactKind::DeferredBundle), 1024)
                .await
                .unwrap_err(),
    ...
    async fn stable_error_classes_cover_missing_unavailable_too_large_and_backend() {
        ...
        let missing = ArtifactRecord::new(
            key.clone(),
            ArtifactLocator::from_stored(
                directory
                    .path()
                    .join("bundles/cap-artifact-test.llmcapture")
                    .to_string_lossy(),
            )
            .unwrap(),
            1,
