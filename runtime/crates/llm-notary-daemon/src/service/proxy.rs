...
pub async fn run(args: ProxyArgs) -> Result<()> {
    ...
    {
        bail!("hosted notary discovery requires LLM_NOTARY_API_KEY or LLM_NOTARY_API_KEY_FILE");
    }
    if cluster_runtime.is_some() && std::env::var_os("LLM_NOTARY_DESKTOP_CONTROL_STDIN").is_some() {
...
async fn proxy_inner(state: AppState, request: Request) -> Result<Response> {
    ...
    let capture_claim = state
        .cluster_runtime
        .as_ref()
        .map(|cluster_runtime| cluster_runtime.capture_claim(capture_id.clone()));
