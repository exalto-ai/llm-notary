...
async fn daemon_uses_the_versioned_loopback_api_for_reads_and_mutations() {
    ...
    let capture: serde_json::Value = client
        .get(format!("http://{admin}/v1/captures/cap-daemon-cli"))
        .send()
        .await
        .unwrap()
        .error_for_status()
    ...
    let finalize: serde_json::Value = client
        .post(format!(
            "http://{admin}/v1/captures/cap-daemon-cli/finalizations"
        ))
        .send()
        .await
        .unwrap()
