...
notarization work, notarized traces, and failures. Recent activity uses safe
event summaries and links back to the relevant work.

![Local dashboard in light mode showing an online service, available vault and notary, capture-state counts, the next notarization action, and recent activity.](images/local-dashboard/overview-light.png)
...
- **Notarizations** shows durable queued, running, interrupted, failed, and
  completed proof operations.
- **Notarized traces** presents the portable `.llmtrace` package, makes its
...
capture state, notarization state, streaming mode, or time. A result always has
...
artifact availability, hashes, and prior notarization state. It never decrypts
or renders the source capture. The notarization history links every durable
operation for that capture to its detailed attempt history.

![Local dashboard in dark mode with Captures selected, provider and notarization filters, a running Anthropic capture selected in the results, privacy-aware previews, lifecycle, and artifact details.](images/local-dashboard/captures-dark.png)

A captured response that has not yet been notarized has one **Notarize** action. The service responds with a
durable operation, and the dashboard moves to its notarization view. Repeating
...
but the dashboard replaces **Notarize** with the provider status and the stable
...
**Retry notarization** appears only when the service marks the operation
`retryable`; retry requeues the same durable operation.

![Notarizations view with a failed benchmark operation selected, its attempt and safe notary-capacity failure code visible, and a Retry notarization action.](images/local-dashboard/notarization-retry.png)

## Verify a notarized trace

Select **Notarized traces**, choose a capture, and inspect four document-like
views:
...
event. Existing captures and notarization work remain available in either
mode.
...
![Mobile local dashboard with a private capture detail behind the open full-height navigation drawer, including ready-to-notarize and active-work counts and all dashboard destinations.](images/local-dashboard/mobile-navigation.png)
...
![Mobile capture detail in a single-panel layout with an All captures back action, Notarize button, lifecycle, and safe metadata visible.](images/local-dashboard/mobile-capture-detail.png)
...
notarization, sharing confirmation, and mobile images use Light mode. Captures,
...
Notarization advances deterministically from queued to running to notarized as
the operation is polled. The fixture does not add the capture to Notarized
