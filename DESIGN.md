...
Use artifact names precisely. A `.llmbundle` is private deferred state, a `.llmtrace` is the portable evidence package, and a bare Library trace is admitted output for inspection. Never label a bundle verified or imply that the bare public JSON carries the package's cryptographic evidence. Put the request/response-body disclosure warning beside every upload or share action.
