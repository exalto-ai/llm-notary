...
The Library and documentation share a `1320px` maximum content width, 38px desktop side padding, and a compact top offset below the navigation. The Library is a browse workspace: start immediately with search, filters, and topic chips rather than a marketing hero. On desktop, use a viewport-height master-detail workspace in roughly a 0.8 / 1.2 column grid with a 32px gutter; the result list and wider inspector scroll independently beneath shared controls. On mobile, return to one natural document scroll. Documentation uses the same outer width and gutter, with a compact sticky navigation column and a constrained reading column; it should feel like a sibling view, not a separate site. At the mobile breakpoint, replace the desktop sidebars with a sticky toolbar below the product header: search occupies the full width, while separate `Docs` and `On this page` controls reveal a flat, bounded-height navigation panel. The panel overlays the document rather than reflowing it and remains internally scrollable on short screens.
...
- **Cards and lists:** use raised surfaces, square corners, and 1px borders. Trace rows and collection entries should expose evidence context through compact metadata and clear scanning hierarchy. Show a `Verified` marker only where it distinguishes the item or confirms a completed check; do not repeat it on every row of an all-verified list. Do not call the collection status “platform-stamped” in UI labels; the stamp is the underlying proof mechanism, while `Verified` is the reader-facing state.
- **Structured grids:** numbered trust cards and documentation card grids share a compact rhythm: `20px` internal padding, roughly `24px` between the metadata row and heading, and no empty height added merely to make the cards feel substantial.
...
- **Forms and dialogs:** retain square input fields, visible borders, and a strong focus ring. Modal backdrops may dim the page, but avoid blur and offset-shadow effects; the dialog remains a flat, paper-like surface.
